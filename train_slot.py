...
def run(args, device_id, error_queue):
    ...
    try:
        ...
        train_slot_single(args, device_id)
...
def train_slot(args, device_id):
    if (args.world_size > 1):
        train_slot_multi(args)
    else:
        train_slot_single(args, device_id)


def train_slot_multi(args):
...
def train_slot_single(args, device_id):
    ...
    def train_iter_fct():
        return data_slot_loader.Dataloader(args, load_dataset(args, 'train', shuffle=True), 
...
def validate_slot(args, device_id):
    timestep = 0
    if args.test_from != "":
...
def validate(args, device_id, pt, step):
    ...
    valid_iter = data_slot_loader.Dataloader(args, load_dataset(args, 'validation', shuffle=False),
...
def test_slot(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    test_iter = data_slot_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False),
