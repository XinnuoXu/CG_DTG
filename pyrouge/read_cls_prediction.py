...
def print_topk_binary(selected_examples):
    gtruths = []; preds = []
    for example_obj in selected_examples:
        clusters = example_obj['clusters']
        sent_scores = example_obj['verdict_scores']
        ...
        verd_labels = np.array(example_obj['verdict_labels'])
        pros_labels = np.array(example_obj['pros_labels'])
        cons_labels = np.array(example_obj['cons_labels'])
        gtruth = (verd_labels+pros_labels+cons_labels) > 0
        gtruth = gtruth.tolist()

        preds.extend(pred)
        gtruths.extend(gtruth)

    gtruths = np.array(gtruths)
    preds = np.array(preds)
    print ('Binary selection (P/R/F)', precision_recall_fscore_support(gtruths, preds, average='binary', pos_label=1))
...
if __name__ == '__main__':
    ...
    raw_path = './outputs.ama/logs.selector_select/'
    json_f = pjoin(raw_path, 'test.res.json')
    example_map = preprocess_for_later_version(json_f)
    ...
    print_topk_binary(example_map.values())
