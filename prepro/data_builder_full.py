...
def _process_slot_summ(params):
    ...
    for d in jobs:
        ...
        # source side
        src = [s.lower() for s in d['document_segs']]
        source_tokens, src_txt = data_obj.preprocess_src(src, args.max_src_ntokens)

        source_tokens_sents = []
        for s in src:
            source_tokens_sent, _ = data_obj.preprocess_src([s], args.max_src_ntokens)
            source_tokens_sents.append(source_tokens_sent)
...
def format_for_slot_summ(args):
    ...
    for corpus_type in datasets:
        ...
        print(a_lst)
        pool = Pool(args.n_cpus)
        for d in pool.imap(_process_slot_summ, a_lst):
            pass
