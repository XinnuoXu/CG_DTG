...
class ReportMgrBase(object):
    ...
    def report_training(self, step, num_steps, learning_rate,
        ...
        if step % self.report_every == 0:
            if multigpu:
                report_stats = \
                    StatisticsCLS.all_gather_stats(report_stats)
            self._report_training(
                step, num_steps, learning_rate, report_stats)
            self.progress_step += 1
            return StatisticsCLS()
        else:
            return report_stats
...
class ReportMgrCLS(ReportMgrBase):
    def __init__(self, report_every, start_time=-1., tensorboard_writer=None):
        ...
        """
        super(ReportMgrCLS, self).__init__(report_every, start_time)
...
class StatisticsCLS(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, n_docs=0, n_correct=0, loss_verd=0.0, loss_pros=0.0, loss_cons=0.0):
        self.loss = loss
        self.n_docs = n_docs
        self.start_time = time.time()
        self.loss_verd = loss_verd
        self.loss_pros = loss_pros
        self.loss_cons = loss_cons
    ...
    def all_gather_stats(stat, max_size=4096):
        ...
        """
        stats = StatisticsCLS.all_gather_stats_list([stat], max_size=max_size)
        return stats[0]
    ...
    def update(self, stat, update_n_src_words=False):
        ...
        self.loss_verd += stat.loss_verd
        self.loss_pros += stat.loss_pros
        self.loss_cons += stat.loss_cons
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        logger.info(("Step %s; xent: %4.5f; " + "lr: %s; %3.0f docs/s; %6.0f sec")
                     % (step_fmt,
                       ...
                       self.n_docs / (t + 1e-5),
                       time.time() - start))
        logger.info(("Step %s; [ATTN] loss_verd: %0.5f; loss_pros: %0.5f; loss_cons %0.5f;")
                     % (step_fmt, 
                        self.loss_verd/self.n_docs, 
                        self.loss_pros/self.n_docs, 
                        self.loss_cons/self.n_docs))
        sys.stdout.flush()
