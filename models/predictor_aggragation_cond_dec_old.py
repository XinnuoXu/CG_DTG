...
class Translator(object):
    ...
    def _run_conditioned_text_generation(self, src, mask_src, src_examples,
        ...
        for step in range(max_length):
            ...
            decoder = self.model.abs_model.get_decoder()
            decoder_outputs = decoder(input_ids=decoder_input,
            ...
            log_probs = self.model.abs_model.lm_head(dec_out)
            log_probs = gen_func(log_probs)
            '''
            sequence_output = decoder_outputs.last_hidden_state
            sequence_output = sequence_output * (self.model.abs_model.model_dim**-0.5)
            log_probs = self.model.abs_model.lm_head(sequence_output)
