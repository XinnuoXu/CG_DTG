...
log_path=/rds/user/hpcxu1/hpc-work/outputs.webnlg/${ntriple}triple.full/logs.re.nn.${tokenizer}/test.res.${test_from}
#for i in $(seq 0.0 .02 1.0)
for i in $(seq 0.0 .1 1.0)
do
	sh ./scripts_d2t.ntriple_full/test_nn.sh ${ntriple} ${tokenizer} ${test_from} ${test_unseen} ${i}
