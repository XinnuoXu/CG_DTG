...
for i in $(seq 2 2 30)
do
	sh ./scripts_d2t.less_triple/test_deterministic.sh $i
...
do
	sh ./scripts_d2t.less_triple/test_nn.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
...
for i in $(seq 0.5 .05 1.5)
do
	sh ./scripts_d2t.less_triple/test_reinforce_graph_nn.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
...
for i in $(seq 0.5 .05 1.5)
do
	sh ./scripts_d2t.less_triple/test_reinforce_graph_nn_sample.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
