...
from nlgi_eval.external.tgen_data import DA
from nlgi_eval.external.e2e_metrics_tsv import read_tsv
from nlgi_eval.external.webnlg_entry import Triple
import nlgi_eval.external.webnlg_parser as webnlg_parser


TEMPLATE_PATHS = {
    'e2e': './tool/nlgi_eval/templates/e2e_templates.json',
    'webnlg': './tool/nlgi_eval/templates/webnlg_templates.json',
}
...
class Evaluator:
    ...
    def triples_to_templates(self, tripleset):
        ...
        for triple in tripleset:
            triple = triple.split('|')
            triple_predicate = triple[1]
            triple_object = triple[2]
            triple_subject = triple[0]
            if triple_predicate not in self.templates:
                # if template isn't found, check remapping first
                if triple_predicate in TEMPLATE_REMAP and TEMPLATE_REMAP[triple_predicate] in self.templates:
                    template = self.templates[TEMPLATE_REMAP[triple_predicate]]
                else:  # if remapping doesn't work either, create a backoff template
                    template = 'The %s of <subject> is <object>.' % triple_predicate
                    template = re.sub('([a-z])([A-Z])', r"\1 \2", template)  # get rid of camel case
                    logger.warn('Created backoff template for %s' % (triple_predicate))
                    self.templates[triple_predicate] = template
            else:  # take the template
                template = self.templates[triple_predicate]
            if isinstance(template, dict):
                template = template[triple_object]
            if isinstance(template, list):
                template = template[0]  # XXX don't take the first, but the best template
            template = template.replace('<subject>', triple_subject)
            obj_str = re.sub('^["\'](.*)["\']$', r'\1', triple_object)  # remove quotes around values
            template = template.replace('<object>', obj_str)
            template = template.replace('_', ' ')
            output.append(template)
            if self.e2e_ignore_restaurant and triple_predicate in ['eat_type', 'eatType'] and obj_str == 'restaurant':
    ...
    def eval_file(self, in_fname):
        ...
        for line in open(in_fname):
            ...
            data.append((json_obj['input_triples'], json_obj['prediction']))
        outputs = []
        for mr, sent in data:
