...
for i in $(seq 8 2 30)
do
	sh ./scripts_d2t/test_deterministic.sh $i
...
for i in $(seq 0.0 .05 1.0)
do
	sh ./scripts_d2t/test_reinforce.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
...
for i in $(seq 0.6 .05 1.5)
do
	sh ./scripts_d2t/test_nn.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
...
#for i in $(seq 0.1 .005 0.25)
for i in $(seq 0.5 .05 1.5)
do
	sh ./scripts_d2t/test_reinforce_graph_nn.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
...
for i in $(seq 0.5 .05 1.5)
do
	sh ./scripts_d2t/test_reinforce_graph_nn_sample.sh $i
	echo "====== test_graph_selection_threshold: $i ======"
	sh ./tool/multi-bleu.sh $log_path
	python ./pyrouge/read_clusters_multiref.py "$log_path.cluster"
	echo ""
